...
def api_pdf_to_word():
    ...
    try:
        ...
        current_app.logger.exception("PDF to Word conversion failed")
        return jsonify({"error": f"Couldn't convert this PDF: {e}"}), 500
    filename = (file.filename.rsplit(".", 1)[0] if "." in file.filename else "Converted") + ".doc"
    return Response(docx_bytes, mimetype="application/msword",
                    headers={"Content-Disposition": f"attachment; filename={filename}"})
...
def api_word_to_pdf():
    ...
    if not file.filename.lower().endswith((".docx", ".doc")):
        return jsonify({"error": "Please upload a .docx or .doc file."}), 400
    try:
        pdf_bytes = word_to_pdf_engine(file.read())
    except Exception as e:
        current_app.logger.exception("Word to PDF conversion failed")
        return jsonify({"error": f"Couldn't convert this document: {e}"}), 500
...
@ai_browser_bp.route("/url-to-file")
def url_to_file_page():
    return render_template("tools/url_to_file.html")

@ai_browser_bp.route("/api/url-to-file", methods=["POST"])
def api_url_to_file():
    from flask import Response
...
@ai_browser_bp.route("/diff-checker")
def diff_checker():
